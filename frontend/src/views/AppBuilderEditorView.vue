...
const STATIC_CAPS = {
  ...
  'output.table':              { label: 'Table View',      icon: 'mdi-table',                 color: '#94a3b8', category: 'Output',    inputs: ['classification_result','regression_result'], configSchema: [{ key: 'max_rows', label: 'Max Rows', type: 'number', default: 100 }, { key: 'show_confidence', label: 'Show Confidence', type: 'toggle', default: true }] },
  ...
  'output.prediction_sink':    { label: 'Prediction Sink', icon: 'mdi-database-export',       color: '#a78bfa', category: 'Output',    inputs: ['classification_result','regression_result','anomaly_result'], description: 'Persist each inference result to a daily CSV file and/or publish it as an MQTT message', tags: ['Persist','CSV','MQTT'], configSchema: [{ key: 'mode', label: 'Output mode', type: 'select', options: ['csv','mqtt','both'], default: 'csv' }, { key: 'mqtt_topic', label: 'MQTT topic', type: 'text', default: 'predictions/{slug}' }, { key: 'csv_dir_hint', label: 'CSV path (info)', type: 'text', default: 'shared/predictions/{slug}/YYYY-MM-DD.csv' }] },
