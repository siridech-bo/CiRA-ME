...
def get_model_artifact(model_name):
    """Get the trained model files for a specific model.
    Returns ONNX for TI NN models, C header for emlearn models.
    If format=ccs_package, returns a full CCS-ready zip with C headers.
    """
        ...
        if os.path.isdir(model_dir):
            for root, dirs, files in os.walk(model_dir):
                for f in files:
                    ...
                    if f == 'user_input_config.h':
                        config_path = os.path.join(root, f)
                    if f.endswith('_model.h'):
                        header_path = os.path.join(root, f)
            break

    if not onnx_path and not header_path:
        return jsonify({'error': f'Model {model_name} not found'}), 404

    # CCS package: zip with C headers + ONNX + config
    if fmt == 'ccs_package':
        import zipfile as _zf
        safe_name = model_name.lower().replace(' ', '_').replace('-', '_')
        zip_path = os.path.join('/tmp', f'ti_nn_{safe_name}.zip')
        template_dir = '/app/ccs_templates'

        with _zf.ZipFile(zip_path, 'w', _zf.ZIP_DEFLATED) as zf:
            # 1. ONNX model
            if onnx_path:
                zf.write(onnx_path, f'model/{safe_name}.onnx')

                # 2. Generate C weight header from ONNX
                try:
                    c_header = _onnx_to_c_header(onnx_path, safe_name)
                    zf.writestr(f'model/{safe_name}_weights.h', c_header)
                except Exception as e:
                    print(f'Warning: ONNX to C conversion failed: {e}')

            # 3. Input config header
            if config_path:
                zf.write(config_path, f'model/{safe_name}_config.h')

            # 4. emlearn C header (if available)
            if header_path:
                zf.write(header_path, f'model/{os.path.basename(header_path)}')

            # 5. CCS main template
            main_template = os.path.join(template_dir, 'common', 'cira_main.c')
            if os.path.exists(main_template):
                zf.write(main_template, 'src/cira_main.c')
            ...
            # 7. README
            is_npu = 'NPU' in model_name.upper()
            zf.writestr('README.txt', f"""CiRA ME - TI NN Model Package for CCS
======================================

Model: {model_name}
Type:  {'NPU-optimized' if is_npu else 'CPU'} Neural Network

Files:
  model/{safe_name}.onnx          - Trained ONNX model
  model/{safe_name}_weights.h     - Model weights as C arrays
  model/{safe_name}_config.h      - Input/output configuration
  src/cira_main.c                 - Firmware template (SCI UART + inference)
  tools/cira_serial_test.py       - Python serial test tool

CCS Integration:
  1. Create CCS project for your TMS320 device
  2. Add model/ files to your project include path
  3. {'Link TI NN runtime library (libtinn.a) from C2000Ware' if is_npu else 'Include weights header and implement forward pass'}
  4. Add src/cira_main.c or write your own main
  5. Build and flash

{'NPU Note:' if is_npu else 'CPU Note:'}
  {'This model is optimized for the NPU accelerator on F28P55x/MSPM0+ devices.' if is_npu else 'This model runs on the C28x CPU core. No NPU required.'}
  {'Use TI NN Compiler in CCS to compile the ONNX model for NPU execution.' if is_npu else 'The weights header contains all parameters as C arrays.'}
  {'The ONNX file can be compiled with: ti_nn_compiler --model model.onnx --target F28P55' if is_npu else ''}

Generated by CiRA ME v1.0.0
""")

        return send_file(zip_path, as_attachment=True,
                        download_name=f'ti_nn_{safe_name}_ccs.zip')

    # Raw format: return single file
    if header_path:
        return send_file(header_path, as_attachment=True,
                        download_name=os.path.basename(header_path))
    if onnx_path:
        return send_file(onnx_path, as_attachment=True,
...
def _onnx_to_c_header(onnx_path, model_name='cira_nn'):
    """Convert ONNX model weights to C header file for CCS integration."""
    import onnx
    ...
    lines = []
    lines.append(f'/**')
    lines.append(f' * CiRA ME - TI NN Model Weights')
    lines.append(f' * Auto-generated from ONNX model')
    lines.append(f' * Model: {model_name}')
    lines.append(f' */')
    lines.append(f'#ifndef {model_name.upper()}_WEIGHTS_H_')
    lines.append(f'#define {model_name.upper()}_WEIGHTS_H_')
    lines.append(f'')
    ...
    lines.append(f'// Input shape: {inp_shape}')
    lines.append(f'// Output shape: {out_shape}')
    lines.append(f'#define {model_name.upper()}_INPUT_SIZE {" * ".join(str(d) for d in inp_shape if d > 0)}')
    lines.append(f'#define {model_name.upper()}_OUTPUT_SIZE {" * ".join(str(d) for d in out_shape if d > 0)}')
    lines.append(f'')
    ...
    for init in model.graph.initializer:
        ...
        lines.append(f'// {init.name}: shape={list(arr.shape)}, {arr.size} params')
        lines.append(f'static const float {model_name}_{safe}[{arr.size}] = {{')
    ...
    lines.append(f'#define {model_name.upper()}_TOTAL_PARAMS {total_params}')
    lines.append(f'')

    # Network structure info
    lines.append(f'// Network structure')
    lines.append(f'typedef struct {{')
    lines.append(f'    const char *name;')
    lines.append(f'    const float *data;')
    lines.append(f'    int size;')
    lines.append(f'    int shape[4];')
    lines.append(f'    int ndim;')
    lines.append(f'}} {model_name}_layer_t;')
    lines.append(f'')
    lines.append(f'static const {model_name}_layer_t {model_name}_layers[] = {{')

    for init in model.graph.initializer:
        arr = numpy_helper.to_array(init)
        safe = init.name.replace('.', '_').replace(':', '_')
        shape_str = ', '.join(str(d) for d in arr.shape)
        pad = ', '.join(['0'] * (4 - len(arr.shape)))
        if pad:
            shape_str += ', ' + pad
        lines.append(f'    {{"{init.name}", {model_name}_{safe}, {arr.size}, {{{shape_str}}}, {len(arr.shape)}}},')

    lines.append(f'}};')
    lines.append(f'#define {model_name.upper()}_NUM_LAYERS {len(model.graph.initializer)}')
    lines.append(f'')
    lines.append(f'#endif /* {model_name.upper()}_WEIGHTS_H_ */')
    lines.append(f'')
